...
:triangular_flag_on_post: **`22 Jan 2019`**: We are developing a highly-effective and efficient distributed training schema with multi-GPUs under PyTorch, supporting not only the backbone (supported by the current PyTorch implementation), but also the head with fully-connected layers, to facilitate high-performance large-scale face recognition (huge number of identities, *e.g.*, >100k). We will add such support into our repo soon.
