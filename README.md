...
``` 
* Face alignment API (perform face detection, landmark localization and alignment with affine transformations on a whole database folder with the directory structure as desmontrated in Sec. [Usage](#Usage), and store the aligned results to a new folder with the same directory structure): ```face_align.py```. 
