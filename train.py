...
if __name__ == '__main__':
    ...
    NUM_EPOCH_WARM_UP = cfg['NUM_EPOCH_WARM_UP']  # use the firt NUM_EPOCH_WARM_UP (5 epoches) to warm up
    NUM_EPOCH = cfg['NUM_EPOCH'] # total epoch number
    ...
    # create a weighted sampler to process imbalanced data
    ...
    batch = 0  # batch index
    NUM_BATCH_WARM_UP = len(train_loader) * NUM_EPOCH_WARM_UP # use the first NUM_BATCH_WARM_UP (5 epochs) to warm up
    ...
    for epoch in range(NUM_EPOCH): # start training process
        ...
        for inputs, labels in tqdm(iter(train_loader)):
            ...
            if batch <= NUM_BATCH_WARM_UP - 1: # use the firt NUM_BATCH_WARM_UP batches (5 epochs) to warm up
            ...
            if (batch % DISP_LOSS_FREQ == 0) and batch != 0: # dispaly training loss & acc every DISP_LOSS_FREQ
                ...
                if batch <= NUM_BATCH_WARM_UP - 1:
                    print("During  Warm Up Process, Batch {}/{}".format(batch, NUM_BATCH_WARM_UP - 1))
        ...
        if epoch <= NUM_EPOCH_WARM_UP - 1:
            print("During  Warm Up Process, Epoch {}/{}".format(epoch, NUM_EPOCH_WARM_UP - 1))
