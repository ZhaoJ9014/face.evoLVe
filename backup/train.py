...
if __name__ == '__main__':
    ...
    OPTIMIZER = optim.SGD([{'params': BACKBONE.parameters()},
                           {'params': HEAD.parameters()}], lr=LR, momentum=MOMENTUM, weight_decay=WEIGHT_DECAY)
    print("=" * 60)
    print(OPTIMIZER)
    ...
    DISP_FREQ = 2000 # frequency to display training loss & acc & save checkpoints
    batch = 0  # batch index
    ...
    for epoch in range(NUM_EPOCH): # start training process
        ...
        for inputs, labels in tqdm(iter(train_loader)):
            ...
            if batch == STAGES[0]: # adjust LR for each training stage, you can also choose to adjust LR manually (with slight modification) once plaueau observed
            ...
            if ((batch + 1) % DISP_FREQ == 0) and batch != 0:
                ...
                # perform validation & save checkpoints per DISP_FREQ
                # validation statistics per DISP_FREQ (buffer for visualization)
                print("=" * 60)
                ...
                buffer_val(writer, "VGGFace2_FP", accuracy_vgg2_fp, best_threshold_vgg2_fp, roc_curve_vgg2_fp,
                           batch + 1)
                print(
                    "Epoch {}/{}, Batch {}/{}, Evaluation: LFW Acc: {}, CFP_FF Acc: {}, CFP_FP Acc: {}, AgeDB Acc: {}, CALFW Acc: {}, CPLFW Acc: {}, VGG2_FP Acc: {}".format(
                        epoch + 1, NUM_EPOCH, batch + 1, len(train_loader) * NUM_EPOCH, accuracy_lfw, accuracy_cfp_ff, accuracy_cfp_fp, accuracy_agedb, accuracy_calfw, accuracy_cplfw, accuracy_vgg2_fp))
                print("=" * 60)

                # save checkpoints per DISP_FREQ
                if MULTI_GPU:
